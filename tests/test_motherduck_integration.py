...
class TestDestinationFactory:
    ...
    def test_motherduck_destination_creation(self, mock_motherduck: MagicMock) -> None:
        ...
        # Verify it was called with database parameter from config
        mock_motherduck.assert_called_once_with(database='sports_analytics_dev')
        assert destination is not None

    @patch('dlt.destinations.motherduck')
    def test_motherduck_destination_error_handling(self, mock_motherduck: MagicMock) -> None:
