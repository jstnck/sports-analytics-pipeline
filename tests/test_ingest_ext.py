...
from sports_analytics_pipeline.ingest import (
    ...
    ingest_season,
    ingest_teams_from_schedule,
    _parse_players_from_summary,
    ingest_players_for_event,
...
def test_scrape_boxscores_for_date_with_cache(tmp_path: Path) -> None:
    ...
    out = ingest_boxscores_for_date(dt, session=None, delay=0, cache_dir=str(cache))
...
def test_scrape_season_single_date(tmp_path: Path) -> None:
    ...
    df = ingest_season(2025, start=dt, end=dt, session=None, delay=0, cache_dir=str(cache))
...
def test_scrape_teams_from_schedule() -> None:
    ...
    teams = ingest_teams_from_schedule(df)
...
def test_scrape_players_for_event_with_cache(tmp_path: Path) -> None:
    ...
    df = ingest_players_for_event(event_id, session=None, cache_dir=str(cache))
    assert not df.empty
...
def test_scrape_players_for_date_uses_scoreboard(tmp_path: Path) -> None:
    ...
    df = ingest_players_for_date(dt, session=None, delay=0, cache_dir=str(cache))
